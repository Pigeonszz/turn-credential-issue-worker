...
function addCorsHeaders(response, origin) {
  if (!origin) return response; // No CORS headers if origin not allowed
  ...
  // For non-preflight responses, only add these headers
...
export default {
  ...
  async fetch(request, env, ctx) {
    // Handle CORS first
    const { allowedOrigin, allowedOrigins } = handleCors(request, env);
    ...
    try {
      ...
      // Return the credentials in the new format
      return addCorsHeaders(
        new Response(JSON.stringify({
